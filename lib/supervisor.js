...
function run (args) {
    var arg, next, watch, ignore, pidFilePath, program, extensions, executor, poll_interval, debugFlag, debugBrkFlag, debugBrkFlagArg, harmony, inspect, cwd;
    ...
    startChildProcess = function() { startProgram(program, executor, cwd); };
...
function help () {
    ...
    ("")
    ("  -c|--cwd <dir>")
    ("    Set the current working directory of supervisor and the child process.")
    ("    Default is $PWD.")
    ("")
    ("  -h|--help|-?")
...
function startProgram (prog, exec, cwd) {
